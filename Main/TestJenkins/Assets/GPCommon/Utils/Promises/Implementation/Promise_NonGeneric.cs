...
namespace GPCommon
{
	...
	/// Implementation downloaded from https://github.com/Real-Serious-Games/C-Sharp-Promise
	...
	public interface IPromise
	{
		...
		/// </summary>
		IPromise<TConverted> Then<TConverted>(Func<IPromise<TConverted>> onResolved, Action<Exception> onRejected);
...
//            Argument.NotNull(() => onRejected);
		...
		/// Add a resolved callback that chains a value promise (optionally converting to a different value type).
		/// </summary>
		public IPromise<TConverted> Then<TConverted>(Func<IPromise<TConverted>> onResolved)
		{
		...
		/// The resolved callback chains a value promise (optionally converting to a different value type).
		/// </summary>
		public IPromise<TConverted> Then<TConverted>(Func<IPromise<TConverted>> onResolved, Action<Exception> onRejected)
		{
...
//            Argument.NotNull(() => onResolved);
			...
			Action resolveHandler = () =>
			{
				onResolved()
					.Then(
						...
						(TConverted chainedValue) => resultPromise.Resolve(chainedValue),
		...
		/// It is rejected as soon as any of the promises have been rejected.
		/// </summary>
		public IPromise<IEnumerable<TConverted>> ThenAll<TConverted>(Func<IEnumerable<IPromise<TConverted>>> chain)
		{
			return Then(() => Promise<TConverted>.All(chain()));
		}
		...
		/// Returns a promise that resolves when the first of the promises has resolved.
		/// </summary>
		public IPromise<TConverted> ThenRace<TConverted>(Func<IEnumerable<IPromise<TConverted>>> chain)
		{
			return Then(() => Promise<TConverted>.Race(chain()));
		}
